<template>
    ...
    <v-row>
        <v-col cols="3">
            <v-select v-model="ano" :items="anos" label="Visualizar por ano" clearable data-test="selectAno"></v-select>
        </v-col>
        <v-col cols="3" class="d-flex align-start">
            <v-btn class="custom-width text-none" color="primary" variant="flat" dark data-test="buttonBuscar">Buscar</v-btn>
        </v-col>
    </v-row>
...
const onFolhaClick = (id: string) => {
    router.push(`/pagamento/AutorizacaoFolha/${id}`);
};
...
onMounted(async () => {
    ...
    await getFolhas();
    if (params.rejeitado) snackbarRejeitada();
...
function snackbarRejeitada() {
  snackbarText.value = `Folha rejeitada com sucesso.`;
  snackbarColor.value = '';
  snackbar.value = true;
}
