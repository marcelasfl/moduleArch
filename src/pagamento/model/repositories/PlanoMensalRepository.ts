...
export default class PlanoMensalRepository implements IRepository {
  ...
  createEhAtualRoute() {
    return new PlanoMensalRoutes({}).ehAtual;
  }

  createEhAnteriorRoute(mes: string | number) {
    return new PlanoMensalRoutes({mes: mes}).anterior;
  }

  createEhPosteriorRoute(mes: string | number) {
    return new PlanoMensalRoutes({mes: mes}).posterior;
  }
...
async fetchPlanoMensalById(id: string | number): Promise<PlanoMensal> {
  ...
  async getPlanoMensalAtual() {
    try {
      const ehAtualRoute = this.createEhAtualRoute();
      const response = await this.apiClient.get(ehAtualRoute);
      return response.data.value[0];
    } catch (error) {
      log.error("Erro ao encontrar plano mensal atual", error);
      throw error;
    }
  }

  async getPlanoMensalAnterior(mes: string | number) {
    try {
      const ehAtualRoute = this.createEhAnteriorRoute(mes);
      const response = await this.apiClient.get(ehAtualRoute);
      return response.data.value[0];
    } catch (error) {
      log.error("Erro ao encontrar plano mensal atual", error);
  ...
  async getPlanoMensalPosterior(mes: string | number) {
    try {
      const ehAtualRoute = this.createEhPosteriorRoute(mes);
      const response = await this.apiClient.get(ehAtualRoute);
      return response.data.value[0];
    } catch (error) {
      log.error("Erro ao encontrar plano mensal atual", error);
