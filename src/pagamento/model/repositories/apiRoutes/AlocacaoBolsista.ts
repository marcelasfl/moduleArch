...
class AlocacaoBolsistaRoutes extends BaseModule{
    constructor(config: BaseConfig) {
        super('pagamentobolsistas', 'alocacaobolsista', config);
    };
    ...
    get expandRota(): string {
        return `${this.entity}?$expand=versaonivel($expand=nivelbolsa($select=sigla)),projeto($expand=edital($expand=areaTecnica,Atividade))`
    }
