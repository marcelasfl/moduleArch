...
class LiberarEditaisRoutes extends BaseModule{
    ...
    get monitorarEdital(): string {
        ...
        return `${this.entity}?$expand=Edital($expand=AreaTecnica,Atividade),PlanoMensal,DecisaoLiberacoes($orderby=horario)&$filter=(PlanoMensal/EhAtual eq true and ${dataAtual} ge PlanoMensal/MarcoGeracaoFolha) or (month(PlanoMensal/Mes) eq ${mesAnterior})`;
    }
};
